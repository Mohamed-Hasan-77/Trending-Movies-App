...
export default function MoviePage() {
    ...
    async function movieTrailer() {
        ...
        try{
        let {data} = await axios.get(`https://api.themoviedb.org/3/movie/${id}/videos?language=en-US`, 
        {params: {
            api_key: 'cffed22b0a485ebb8a8ade2cd4bcd5f1',
        }},  )
        // console.log(data);
    ...
    return <>
        ...
        {movie ? <> 
            ...
            <img src={"https://image.tmdb.org/t/p/w500"+movie.backdrop_path} className='w-full h-full object-cover absolute top-0 left-0 z-0  ' alt="wallpaper"  />         
            ...
            <div className="container px-5 py-5 mb-10 md:mb-0 md:space-x-5 dark:text-white rounded-lg flex justify-center  md:justify-between items-start  shadow-lg flex-col md:flex-row bg-white dark:bg-darkTheme absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 z-10">
                ...
                <div className="poster w-full text-center mb-5 md:w-1/5  md:-translate-y-0 px-3">
                    <div className="img w-full flex justify-center">
                        <img src={"https://image.tmdb.org/t/p/w500"+movie.poster_path} className='rounded hidden md:block md:w-full border-gray-200 border-4' alt="Movie Poster" />
                    </div>
                    ...
                    <div className="movieTitle block md:hidden">
                        ...
                        <h3> <span className=' text-themeColor '> {movie.vote_average} / </span> <span className='text-sm'> {movie.vote_count} Voted </span></h3>
                        ...
                        <div className="buttons flex justify-around md:justify-between gap-3 ">
                            <button className='px-2 py-1 bg-themeColor_purple text-white w-1/2 hover:bg-themeColor transition-all duration-300 '> Like </button>
                            <button className='px-2 py-1 bg-themeColor_red text-white w-1/2 hover:bg-gray-800 transition-all duration-300 ' > Dislike </button>
                        </div>
                ...
                <div className=" w-full md:w-5/6   md:space-y-3 ">
                    <div className="watch-add flex justify-between items-start md:items-center  ">
                        <button className='px-4 py-2 bg-themeColor_purple  text-white hover:bg-gray-800 transition-all duration-300 rounded-full'> <i class="fa-solid fa-play"></i> <a target='_blank' href={movie.homepage}>  Watch Now </a> </button>
                        <button className='px-4 py-2 bg-themeColor_red text-white hover:bg-gray-800 transition-all duration-300  rounded-full '> <i class="fa-solid fa-heart"></i> add To fav </button>
                    </div>
                    ...
                    <div className="rating flex items-center  space-x-5 mt-4">
                        <div onClick={trailerToggle} className="trailerBtn px-2 py-1 border border-black hover:bg-darkTheme hover:text-white cursor-pointer transition-all duration-300">  <i class="fa-solid fa-video"></i>  Trailer   </div>
                    ...
                    <div className="moreInfo flex justify-between ">
                        <div className="col-1 w-1/2">
                            ...
                            <p> <span className='font-bold'> Genre </span>  : {movie.genres?.map((genre, idx) => {
                                return <span key={idx}> {genre.name } , </span>
                            })} </p>
                            <p> <span className='font-bold'> Language </span>  : {movie.spoken_languages?.map((lang, idx) => {
                                return <span key={idx}> {lang.english_name } , </span>
                            })} </p>
                        </div>
                        <div className="col-2 w-1/2 ">
                            <p><span className='font-bold'> Duration </span>    : {movie.runtime}</p>
                            <p> <span className='font-bold'> Country </span>   : {movie.production_countries?.map((country, idx) => {
                                return <span key={idx}> {country.name } , </span>
                            })} </p>
                            <p> <span className='font-bold'> Production Companies </span>  : {movie.production_companies?.map((company, idx) => {
                        ...
                        {movie.poster_path ? <>
                            ...
                            <div className=' hover:translate-y-3 shadow-md  transition-all duration-500 ease-in-out cursor-pointer relative'>
                                <div className="img realtive ">
                                    <Link onClick={() => { newMovieSearch()}}  to={`/moviePage/${movie.id}`}>
                                        {/* <img className='rounded' src={"https://image.tmdb.org/t/p/w500"+movie.poster_path} alt="Movie Poster" /> */}
                                        {movie.poster_path ? <>
                                                <img src={"https://image.tmdb.org/t/p/w500"+movie.poster_path} className='' alt="Movie Poster" />
                                ...
                                <div className='movie-desc bg-white dark:bg-darkTheme px-2  py-3 text-xs  dark:text-white rounded-b  '> 
                                    {movie.title ? <>
                                        ...
                                        <h4 className='font-bold'> {movie.name.split(" ").length > 3 ? movie.name.split(" ").slice(0, 3).join(" ") + "..." :  movie.name}   </h4>
                                        </>}
                                    <div className="dec mt-1 flex justify-between items-center">
                                        {movie.release_date ? <>
                                            <span> {movie.release_date?.split("-").slice(0,1).join("")} </span>
                                        
                                        </> : <>
                                            <span> {movie.first_air_date?.split("-").slice(0,1).join("")} </span>
np
                                        </> }
