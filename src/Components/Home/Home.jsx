...
export default function Home() {
  ...
  async function getTrendingMovies() {
    ...
    try{
      let {data} = await axios.get('https://api.themoviedb.org/3/trending/movie/week?language=en-US', 
      {params: {
  ...
  async function getTrendingTvShows() {
    ...
    try{
      let {data} = await axios.get('https://api.themoviedb.org/3/trending/tv/week?language=en-US', 
      {params: {
        api_key: 'cffed22b0a485ebb8a8ade2cd4bcd5f1',
    }},  )
  ...
  async function getTrendingActors() {
    ...
    try{
      let {data} = await axios.get('https://api.themoviedb.org/3/trending/person/week?language=en-US', 
      {params: {
        api_key: 'cffed22b0a485ebb8a8ade2cd4bcd5f1',
    }},  )
...
<div role="status" className="space-y-8 animate-pulse md:space-y-0 md:space-x-8 md:flex md:items-center mx-auto  md:px-60 h-screen">
...
{/* -------------------------------------------------------------------------------------------------------------------------- */}
        ...
        <div className="heading w-full  flex justify-between items-end mt-5 flex-wrap ">
            ...
            <div className="headers order-2 md:order-1 w-full md:w-1/2 text-center md:text-left ">
              <h2 className=' text-sm mt-3 md:mt-0  md:text-1xl dark:text-white  transition-colors duration-300 hover:text-themeColor_purple '> Trending Movies this week <i className="fa-solid text-sm ml-2 fa-arrow-trend-up"></i> </h2>
            </div>
              ...
              <div className="relative w-full md:w-1/4 order-1 md:order-2 text-lg bg-transparent px-2 md:px-0 text-black  dark:text-white ">
                <div className="flex items-center border-b border-b-2 border-themeColor_purple py-2">
                  <input onChange={() => {searching()}} ref={searchRef} className=" searchInput bg-transparent border-none mr-3 px-4 py-1 leading-tight focus:outline-none outline-none  " type="text" placeholder="Search"/>
                  <button onClick={() => {goToSearchResults()}}  className="absolute right-0 top-0  hover:bg-gray-700  bg-themeColor_purple text-white px-4 py-2">
...
return <div key={idx} className=" w-1/2 md:w-1/5 lg:w-44 p-2    ">
      <div className=' hover:translate-y-3 shadow-md  transition-all duration-500 ease-in-out cursor-pointer relative'>
        ...
        <span className=' text-white text-xs font-bold backdrop-blur-lg px-3 py-1 rounded m-1 absolute top-0 right-0'> {Math.round(movie.vote_average)} <i className="fa-solid fa-star ml-1 text-yellow-400"></i> </span>
  ...
  <Link to="/movies">
      ...
      <div className="w-full h-64 font-bold text-gray-500 hover:text-themeColor_purple bg-gray-300 rounded-lg flex justify-center items-center  dark:bg-gray-600 dark:text-white ">   More... </div>
...
{trendingTV ? <>
  ...
  <div className="heading w-full  flex justify-between items-end mt-16 ">
        ...
        <div className="headers w-full text-center md:text-left  ">
          <h2 className=' md:text-1xl dark:text-white  transition-colors duration-300 hover:text-themeColor_purple'> Trending TV Shows this week <i className="fa-solid text-sm ml-2 fa-tv"></i>  </h2>
        </div>
                  ...
                  <div className=' hover:translate-y-3 shadow-md  transition-all duration-500 ease-in-out cursor-pointer relative'>
                    <div className="img realtive ">
                        <Link to={`/tvPage/${TV.id}`}>  
                          <img className='rounded' src={"https://image.tmdb.org/t/p/w500"+TV.poster_path} alt="" />
                          <div className="imghover rounded absolute top-0 left-0 w-full h-full flex opacity-0 hover:opacity-100 hover:backdrop-brightness-50 justify-center items-center">
                            <i className="fa-solid fa-play text-themeColor_purple text-3xl "></i>  
                          </div>
                        </Link>
                    ...
                    <span className=' text-white text-xs font-bold backdrop-blur-lg px-3 py-1 rounded m-1 absolute top-0 right-0'> {Math.round(TV.vote_average)} <i className="fa-solid fa-star ml-1 text-yellow-400"></i> </span>

                    <div className='movie-desc bg-white dark:bg-darkTheme px-2  py-3 text-xs  dark:text-white rounded-b  '> 
                      <h4 className='font-bold'> {TV.name.split(" ").length > 3 ? TV.name.split(" ").slice(0, 3).join(" ") + "..." :  TV.name}   </h4>
                      <div className="dec mt-1 flex justify-between items-center">
                        <span> {TV.first_air_date.split("-").slice(0,1).join("")} | {TV.origin_country[0].toUpperCase()}</span>
                        <span className=' bg-themeColor_red px-1 py-1 text-white rounded'> {TV.media_type}</span>
                      </div>              
                    </div>
...
<div className="headers w-full text-center md:text-left ">
  <h2 className=' md:text-1xl dark:text-white transition-colors duration-300 hover:text-themeColor_purple'> Trending Actors this week...  </h2>
</div>
...
<div className="wrapper flex flex-wrap justify-start ">
          ...
          return <div key={idx} className="w-1/2 md:w-1/5 lg:w-44 px-2 p-1 my-3  ">
